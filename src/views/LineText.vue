<template>
    <my-page title="线条文字" :page="page">
        <textarea class="form-control" v-model="code" rows="6" placeholder="中文数字"></textarea>
        <div class="btns">
        ...
        <textarea id="result" class="form-control result-input" rows="6" placeholder="中文数字"></textarea>
    </my-page>
</template>
...
<script>
    ...
    function beginGenerator(inputText) {
        ...
        if(validChars == false) {
            alert('错误：字符 "'+invalChar+'" 无效。 只有字母 a-z 数字 0-9 被接受。')
        }
        buildStyle2(inputText)
    }
    ...
    function buildStyle2(inputText,booleanRepeat) {
        ...
        if(newline) {
            ...
            window.result.value = outputText;
    ...
    export default {
        ...
        methods: {
            ...
            calculate() {
                ...
                beginGenerator(this.code)
                // this.result = chinese2Number(this.code)
            }
        }
    }
</script>
